...
## Plot 2 assets
```{r plot assets}
plot.xts(nasdaq, grid.col = 'white',  yaxis.right = FALSE, 
...
# plot returns
nasdaq_return <- diff(log(nasdaq))
tesla_return <- diff(log(tesla))

plot.xts(nasdaq_return)
plot.xts(tesla_return)

# plot dynamic variance
nasdaq_var <- nasdaq_return^2
...
```{r table}
nasdaq_row <- cbind(mean(nasdaq_return, na.rm=TRUE), mean(sqrt(nasdaq_var), na.rm=TRUE))
tesla_row <- cbind(mean(tesla_return, na.rm=TRUE), mean(sqrt(tesla_var), na.rm=TRUE))

table <- data.frame(rbind(nasdaq_row, tesla_row))
colnames(table) <- c("average return", "average sd")
rownames(table) <- c("NASDAQ", "TESLA")
